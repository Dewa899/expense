...
import { Plus, Link as LinkIcon, Camera, History, Wallet, Settings2, Trash2, ListTree, Pencil, ChevronLeft } from "lucide-react";
...
export function FormView(props: FormViewProps) {
	...
	// Fallback headers if not synced to show preview
		...
		>
				...
				<div className="bg-emerald-500 dark:bg-emerald-600 rounded-3xl p-6 text-black shadow-lg shadow-emerald-500/20 relative overflow-hidden group">
					...
					<div className="relative z-10">
						<div className="flex justify-between items-start">
							...
							<Button size="sm" onClick={props.onViewDetail} className="h-8 bg-black/10 hover:bg-black/20 text-black border-none rounded-full font-bold text-[10px] px-3">
								<History size={12} className="mr-1" /> {t("viewDetail")}
							</Button>
						</div>
						<h2 className="text-3xl font-black tracking-tight mt-1">{props.formatCurrency(props.totalAmount)}</h2>
						<div className="flex justify-between items-center mt-6">
							<div className="flex -space-x-2">
								{[1, 2, 3].map((i) => (
									<div key={i} className="w-8 h-8 rounded-full border-2 border-emerald-500 bg-emerald-400 flex items-center justify-center text-[10px] font-bold"><Wallet size={12} /></div>
								))}
							</div>
							<div className="flex gap-2">
								<Dialog open={props.isManageFieldsOpen} onOpenChange={props.setIsManageFieldsOpen}>
										...
										<div className="space-y-4 p-6 pt-2">
											{editingOptionsIdx === -1 && renamingIdx === -1 ? (
													...
													<div className="flex flex-col gap-3">
														...
														<div className="flex flex-col gap-2">
															<div className="flex gap-2">
																...
																<Button onClick={props.onAddField} disabled={props.customFields.length >= 2 || props.loading || !props.newFieldName.trim()} className="bg-emerald-500 text-black font-bold px-6">{t("add")}</Button>
															</div>
															<div className="flex items-center gap-2 px-1"><input type="checkbox" id="newFieldReq" checked={props.newFieldRequired} onChange={(e) => props.setNewFieldRequired(e.target.checked)} className="w-4 h-4 rounded" /><Label htmlFor="newFieldReq" className="text-xs font-medium">{t("isRequired")}</Label></div>
														</div>
													</div>
														...
														{props.customFields.map((field, idx) => (
															<div key={idx} className="flex items-center justify-between p-3 bg-zinc-50 dark:bg-zinc-950 rounded-xl border border-zinc-100 dark:border-zinc-800">
																...
																<div className="flex gap-1">
																	<Button variant="ghost" size="sm" onClick={() => { setRenamingIdx(idx); setRenamingInput(field.name); setRenamingType(field.type); setRenamingRequired(field.required); }}><Pencil size={16} /></Button>
																	{field.type === "dropdown" && (<Button variant="ghost" size="sm" onClick={() => setEditingOptionsIdx(idx)} className="text-emerald-600"><ListTree size={16} /></Button>)}
																	<Button variant="ghost" size="sm" onClick={() => props.onDeleteField(idx, field.name)} className="text-destructive"><Trash2 size={16} /></Button>
																</div>
															</div>
														))}
													</div>
												...
												<div className="space-y-4">
													...
													<Button onClick={() => { props.onRenameField(renamingIdx, renamingInput, renamingType, renamingRequired); setRenamingIdx(-1); }} disabled={props.loading} className="w-full bg-emerald-500 text-black font-bold h-12 rounded-xl mt-2">{t("editField")}</Button>
												</div>
											) : (
												<div className="space-y-4">
													<div className="flex gap-2"><Input placeholder={t("newOption")} value={props.newOptionInput} onChange={(e) => props.setNewOptionInput(e.target.value)} className="rounded-xl" /><Button onClick={() => props.onAddOption(editingOptionsIdx, props.newOptionInput)} className="bg-emerald-500 text-black font-bold rounded-xl">{t("add")}</Button></div>
													<div className="max-h-[200px] overflow-y-auto space-y-2">{(props.customFields[editingOptionsIdx].options || []).map((opt) => (<div key={opt} className="flex items-center justify-between p-3 bg-zinc-50 dark:bg-zinc-950 rounded-xl"><span>{opt}</span><Button variant="ghost" size="sm" onClick={() => props.onDeleteOption(editingOptionsIdx, opt)}><Trash2 size={14} /></Button></div>))}</div>
												</div>
											)}
										</div>
									</DialogContent>
												...
												<div className="space-y-3 w-full">
													...
													<div className="flex flex-col gap-2">
														...
														{props.headers.length > 0 && (
															<Button 
																variant="outline" 
																className="w-full h-10 rounded-xl font-bold text-xs flex items-center justify-center gap-2 border-zinc-200 dark:border-zinc-800"
													...
													<Button variant="ghost" size="sm" className="text-destructive hover:bg-destructive/10 font-bold text-[10px] mt-2" onClick={props.onDisconnect}>{t("googleSyncDisconnect")}</Button>
												</div>
											) : (
												<Button 
			...
			<section className="bg-white dark:bg-zinc-900 rounded-3xl p-6 border border-zinc-200 dark:border-zinc-800 shadow-sm">
				...
				<div className="space-y-5">
								...
								<div key={header} className="space-y-2">
												...
												<DialogContent className="sm:max-w-[400px] rounded-3xl">
													...
													<div className="space-y-4 py-4">
														...
														<div className="max-h-[200px] overflow-y-auto space-y-2">{(isCoreCat ? props.categories : customField?.options || []).map((opt) => (<div key={opt} className="flex items-center justify-between p-3 bg-zinc-50 dark:bg-zinc-950 rounded-xl border border-zinc-100 dark:border-zinc-800">
															<span className="text-sm font-medium">{opt}</span><Button variant="ghost" size="sm" onClick={() => isCoreCat ? props.onDeleteCategory(opt) : props.onDeleteOption(customFieldIdx, opt)}><Trash2 size={14} /></Button></div>))}</div>
													</div>
												</DialogContent>
											</Dialog>
										)}
									...
									{(isCoreCat || (customField?.type === "dropdown")) ? (
										<Select value={props.formData[header] || ""} disabled={props.loading || !props.user} onValueChange={(val) => props.onInputChange(header, val || "")}>
											...
											<SelectContent className="rounded-xl">{(isCoreCat ? props.categories : customField?.options || []).map((opt) => (<SelectItem key={opt} value={opt}>{opt}</SelectItem>))}</SelectContent>
										</Select>
									) : isType ? (
										<Select value={props.formData[header] || ""} disabled={props.loading || !props.user} onValueChange={(val) => props.onInputChange(header, val || "")}>
					...
					{props.user ? (
						<Button disabled={props.loading} onClick={props.onSubmit} className="w-full h-14 bg-emerald-500 hover:bg-emerald-600 text-black font-black text-lg rounded-2xl mt-4 shadow-lg">
							{props.loading ? "..." : t("addExpense")}
						</Button>
					) : (
						<Button onClick={() => setIsSyncModalOpen(true)} className="w-full h-14 bg-zinc-100 hover:bg-zinc-200 dark:bg-zinc-800 dark:hover:bg-zinc-700 text-zinc-900 dark:text-zinc-100 font-black text-lg rounded-2xl mt-4 shadow-sm">
							Sync Account to Add
						</Button>
					)}
				</div>
